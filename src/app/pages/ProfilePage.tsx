...
export function ProfilePage({ images, onNavigate }: ProfilePageProps) {
  ...
  const { totalXP, level, achievements, getCompletedCoursesCount, getCompletedLessonsCount, dailyStreak, addXP } = useXP();
  ...
  const handleClaimXP = () => {
    ...
    setTimeout(() => {
      ...
      if (claimed > 0) {
        addXP(claimed, `Claimed ${claimed} XP from referrals`);
