...
interface FoundersListPageProps {
  currentPage: string;
...
export function FoundersListPage({ currentPage, onNavigate, onLogout }: FoundersListPageProps) {
  ...
  const load = useCallback(() => {
    setState('loading');
    apiClient
      .getPaginated<AdminFounder>('founders/admin', {
        query: {
  ...
  async function handleDelete(id: string, name: string) {
    ...
    try {
      ...
      toast.error(errorMessage(err, 'Failed to delete story'));
    } finally {
      setDeletingId(null);
    }
  }
  ...
  async function handleApprove(id: string) {
    try {
      await apiClient.patch(`founders/${id}`, { status: 'PUBLISHED' });
      toast.success('Story approved and published');
      load();
    } catch (err) {
      toast.error(errorMessage(err, 'Failed to approve story'));
    }
  }
  ...
  async function handleReject(id: string) {
    ...
    try {
      await apiClient.patch(`founders/${id}`, { status: 'DRAFT' });
  ...
  async function handleToggleFeatured(id: string, featured: boolean) {
    try {
      await apiClient.patch(`founders/${id}`, { featured: !featured });
      toast.success(!featured ? 'Story marked as featured' : 'Story removed from featured');
      load();
    } catch (err) {
      toast.error(errorMessage(err, 'Failed to update story'));
    }
  }

  const getStatusBadge = (status: AdminFounder['status']) => {
    switch (status) {
      case 'PUBLISHED':
        return (
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-[#0F0F10]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden md:ml-64">
        <AdminHeader title="Stories Management" onMenuClick={() => setIsMobileSidebarOpen(true)} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
          <div className="mb-4 md:mb-6 flex flex-col sm:flex-row gap-3 md:gap-4 justify-between">
            ...
            <button
              onClick={() => onNavigate('admin/founders/create')}
          ...
          <div className="bg-white dark:bg-[#1A1A1C] rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            {state === 'loading' ? (
              ...
              <div className="py-24 text-center text-sm text-gray-500 dark:text-gray-400">
                Couldn&apos;t load stories. Please try again.
              </div>
            ) : (
              <>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-800/50">
                      <tr>
                        ...
                        <th className="text-left py-3 px-4 text-sm text-gray-600 dark:text-gray-400">Company</th>
                        <th className="text-left py-3 px-4 text-sm text-gray-600 dark:text-gray-400">Status</th>
                        <th className="text-left py-3 px-4 text-sm text-gray-600 dark:text-gray-400">Featured</th>
                    ...
                    <tbody>
                      {founders.map(founder => (
                        <tr key={founder.id} className="border-t border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                          <td className="py-3 px-4">
                            <p className="text-sm text-gray-900 dark:text-gray-100 line-clamp-1">{founder.name}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{founder.role}</p>
                          </td>
                          <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                            {founder.company || '-'}
                          </td>
                          <td className="py-3 px-4">{getStatusBadge(founder.status)}</td>
                          <td className="py-3 px-4">
                            <button
                              onClick={() => handleToggleFeatured(founder.id, founder.featured)}
                              className="p-1.5 hover:bg-yellow-100 dark:hover:bg-yellow-900/20 rounded transition-colors"
                              title={founder.featured ? 'Remove from featured' : 'Mark as featured'}
                            >
                              <Star
                                className={`w-4 h-4 ${
                                  founder.featured
                          ...
                          <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">
                            {founder.publishedAt ? new Date(founder.publishedAt).toLocaleDateString() : '-'}
                          </td>
                          <td className="py-3 px-4">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                onClick={() => onNavigate(`admin/founders/edit/${founder.id}`)}
                ...
                <div className="border-t border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Showing {founders.length === 0 ? 0 : (page - 1) * PER_PAGE + 1}-{(page - 1) * PER_PAGE + founders.length} of {total} stories
                  </p>
