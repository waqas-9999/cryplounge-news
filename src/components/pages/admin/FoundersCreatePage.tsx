...
import { Save, Upload, X, Tag, Sparkles, Image as ImageIcon, Loader2 } from 'lucide-react';

interface FoundersCreatePageProps {
  currentPage: string;
...
export function FoundersCreatePage({ currentPage, onNavigate, onLogout }: FoundersCreatePageProps) {
  ...
  async function handleSave(status: 'draft' | 'published') {
    if (!formData.name.trim() || !formData.role.trim() || !formData.bio.trim() || !formData.excerpt.trim()) {
      toast.error('Name, role, bio and excerpt are required');
      return;
    }
    setSaving(status);
    try {
      const created = await apiClient.post<{ id: string; slug: string }>('founders', {
        name: formData.name,
      ...
      onNavigate(`admin/founders/edit/${created.id}`);
    } catch (err) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-[#0F0F10]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden md:ml-64">
        <AdminHeader title="Create Story" onMenuClick={() => setIsMobileSidebarOpen(true)} />
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-white dark:bg-[#1A1A1C] rounded-xl p-6 border border-gray-200 dark:border-gray-800">
                  ...
                  <textarea
                    ...
                    placeholder="Short standfirst shown on story cards..."
                    rows={3}
                    maxLength={600}
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-[#202225] border border-gray-300 dark:border-gray-700 rounded-lg text-gray-900 dark:text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-400"
                  />
