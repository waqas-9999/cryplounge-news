...
import { listRegulations, type RegulationItem } from '@/services/regulations';
...
export function RegulationPage({ images, onNavigate }: RegulationPageProps) {
  ...
  const regions = Array.from(new Set(items.map(i => i.region).filter((r): r is string => !!r)));
  ...
  if (!loading && !hero) {
    return (
      <main className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 md:py-8">
        ...
        <p className="text-gray-500 dark:text-[#A0A0A5]">No regulation coverage published yet.</p>
      </main>
    );
  }
          ...
          {hero && (
            ...
            >
              <div className="mb-3 md:mb-4">
                <span className="text-[#EFB81A] text-xs md:text-sm">{hero.countryName}</span>
            ...
            <div>
              ...
              <div className="space-y-3 md:space-y-4 bg-white dark:bg-[#1A1A1A] rounded-xl md:rounded-2xl p-4 md:p-6 border border-gray-200 dark:border-gray-800 transition-colors">
                {latest.map((item, index) => (
                  <div key={item.id}>
                    <LatestNewsCard category={item.countryName} categorySlug={item.categorySlug} time={timeAgo(item.publishedAt)} title={item.title} image={imageFor(index)} articleSlug={item.slug} onNavigate={onNavigate} />
                ...
                {currentArticles.map((item, index) => (
                  <div key={item.id}>
                    <div onClick={() => onNavigate?.(`news/${item.categorySlug}/${item.slug}`)} className="cursor-pointer group">
                      <div className="mb-2">
                        <span className="text-[#EFB81A] text-xs md:text-sm">{item.countryName}</span>
          ...
          <div className="bg-[#FFD200] rounded-xl md:rounded-2xl p-4 md:p-6">
            ...
            <form onSubmit={handleNewsletterSubmit} className="space-y-2">
              <input
                ...
                aria-label="Email address to subscribe to regulation alerts"
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {trending.map((item, index) => (
              <TrendingCard key={item.id} category={item.countryName} categorySlug={item.categorySlug} time={timeAgo(item.publishedAt)} title={item.title} tags={item.tags.slice(0, 2)} image={imageFor(index)} articleSlug={item.slug} onNavigate={onNavigate} />
            ))}
          </div>
